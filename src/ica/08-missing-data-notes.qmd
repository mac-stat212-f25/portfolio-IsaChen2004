...
1. **Missing data mechanism** For each of the following situations, propose what missing data mechanism you think is most likely at play. 
  ...
  Missing not at random, because the probability of the missing data is related with some unobserved varaibles, like the unrecorded reasons that makes them dropped out. 
  b. A weather station records temperature, humidity, and wind speed every hour. Some of the recorded values are missing.
  Missing completely at random, the missing data thing is not realted with what we are observing. 
  ...
  Missing completely at random, the bugs is not related with what we are observing and it is also totally random. 
...
naniar::vis_miss(airquality) # Where are NA's located? Mainly in Ozone and Solar.R
